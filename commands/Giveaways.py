...
async def stop_giveaway(self, g_id, data):
    ...
    result_embed = nextcord.Embed(
        title="<a:gv:939799036779716619> {} <a:gv:939799036779716619>".format(data["prize"]),
        color=self.color,
    ...
    json.dump(giveaways, open("db/giveaways.json", "w"), indent=4)


async def reroll_giveaway(self, g_id, data):
    channel = self.bot.get_channel(data["channel_id"])
    giveaway_message = await channel.fetch_message(int(g_id))
    users = await giveaway_message.reactions[0].users().flatten()
    users.pop(users.index(self.bot.user))
    ...
    result_embed = nextcord.Embed(
        title="<a:gv:939799036779716619> {} <a:gv:939799036779716619>".format(data["prize"]),
        color=self.color,
        description="Giveaway rerolled | Congratulations new winner {} you won the giveaway !".format(", ".join(users_mention))
    ) \
        .set_footer(icon_url=self.bot.user.avatar.url, text="Giveaway rerolled !")
    await giveaway_message.edit(embed=result_embed)
    ghost_ping = await channel.send(", ".join(users_mention))
    await ghost_ping.delete()
    giveaways = json.load(open("db/giveaways.json", "r"))
...
class Giveaways(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.config = json.load(open("db/config.json", "r"))
    ...
    async def giveaway(self, ctx: commands.Context):
        init = await ctx.send(embed=nextcord.Embed(
            title="<a:gv:939799036779716619> New Giveaway ! <a:gv:939799036779716619>",
        ...
        for question in questions:
            embed = nextcord.Embed(
                title="Giveaway <a:gv:939799036779716619>",
        ...
        giveaway_embed = nextcord.Embed(
            title="<a:gv:939799036779716619> {} <a:gv:939799036779716619>".format(prize),
            color=self.color,
    ...
    async def greroll(self, ctx: commands.Context, message_id):
        ...
        if not message_id in giveaways.keys(): return await ctx.send(
            embed=nextcord.Embed(title="Error",
                                description="This giveaway ID is not found.",
                                color=self.color))
        await reroll_giveaway(self, message_id, giveaways[message_id])
    ...
    @commands.has_permissions(manage_guild=True)
    async def gclear(self, ctx: commands.Context, g_id):
        giveaways = json.load(open("db/giveaways.json", "r"))
        del giveaways[g_id]
        json.dump(giveaways, open("db/giveaways.json", "w"), indent=4)
        await ctx.send(f"Cleared the message `{g_id}` from **database** successfully 👍🏼")
